...
commonModule.directive('videoTagItem', function(PlayerData) {
    return {
        restrict: 'EA',
...
commonModule.directive('serverForm', function() {
    ...
    return {
        ...
        link: function(scope, elem, attr, form) {
            ...
            form.submit = function(resourceCall, data, successCallback, errorCallback) {
                console.log("Call server form submit with data: " + data);
                if (form._pending) {
                    return;
                }
                form._pending = true;
                resourceCall(data, function(results) {
                    if (!results.success) {
                        var errors;
                        if (errors in results.validationsErrors) {
                            form.setValidationErrors(errors)
                        }
                    }
                    successCallback(results);
                }, errorCallback).$promise.finally(function() {
